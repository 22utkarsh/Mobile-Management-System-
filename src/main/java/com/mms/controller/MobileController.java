...
public class MobileController {
	...
	public ResponseEntity<List<Mobile>> getAllMobilesByModelAndColor(@PathVariable String model,@PathVariable String color) {
	...
	@PutMapping("/mobile/{ime}")
	public ResponseEntity<Mobile> updateMobile(@PathVariable Long ime , @RequestBody Mobile mobile) throws Exception{
		Mobile mob = mobRepo.findById(ime)
				.orElseThrow(() -> new Exception("No Mobile of IME: "+ime+" is found :(("));
		
		mob.setMaker(mobile.getMaker());
		mob.setCamera(mobile.getCamera());
		mob.setColor(mobile.getColor());
		mob.setModel(mobile.getModel());
		mob.setPrice(mobile.getPrice());
		mob.setProcessor(mobile.getProcessor());
		
		Mobile updatedMob = mobRepo.save(mob);
		return ResponseEntity.ok(updatedMob);
	}
	...
	public ResponseEntity<String> deleteMobile(@PathVariable Long ime){
		if(mobServ.deleteMobile(ime)) {
			...
			return new ResponseEntity<String>("UnableToDelete :Mobile with IME :"+ime+" Not found",HttpStatus.NOT_FOUND);
		}
	}
